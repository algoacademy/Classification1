...
Sebagai alternatif, apabila Anda lebih familiar menggunakan gaya coding **base R**, Anda dapat melakukan filter data menggunakan fungsi *conditional subsetting* `data[<kondisi yang dibutuhkan>,]`, lalu simpanlah ke dalam objek `df_left`. Selanjutnya, Anda dapat melakukan agregasi `average_monthly_hours` berdasarkan kolom `division` dari objek `df_left` menggunakan fungsi `aggregate()`. Jangan lupa untuk menggunakan argumen `mean` dalam parameter `FUN` yang terdapat pada fungsi `aggregate()` dan simpanlah ke dalam objek `df_agg`. Untuk mengurutkan nilai rata-rata `average_monthly_hours` dari yang tertinggi ke terendah, Anda dapat menggunakan fungsi `order` dalam *conditional subsetting* `data[order(nama_kolom, decreasing = T), ]`
